...
const selectedEmail = useState<string>(() => props.accounts[0]!.email)
...
<template>
  <Select v-model="selectedEmail">
    <SelectTrigger
      ...
      :class="cn(
        'flex items-center gap-2 [&>span]:line-clamp-1 [&>span]:flex [&>span]:items-center [&>span]:gap-1 [&>span]:truncate [&_svg]:h-4 [&_svg]:w-4 [&_svg]:shrink-0',
        { 'flex h-9 w-9 shrink-0 items-center justify-center p-0 [&>span]:w-auto [&>svg[role=img]]:hidden': isCollapsed },
      )"
    >
