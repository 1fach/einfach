...
export class WebLocksBase {
   ...
   * @param {number} flags
   */
  async #apply(method: (targetState: number) => void, flags: number) {
  ...
  async #unlock(targetState: number) {
    if (targetState === this.#state) return VFS.SQLITE_OK
    switch (this.#state) {
      case VFS.SQLITE_LOCK_EXCLUSIVE:
        switch (targetState) {
   ...
   */
  _acquireWebLock(lockName: string, options: LockOptions) {
   ...
   * @param {string} lockName
   */
  async _pollWebLock(lockName: string | undefined) {
